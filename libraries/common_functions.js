...
function rejecting_promise(data) {
  ...
  return new Promise(function (resolve, reject) {
    if (RESOLVE_ALL) resolve(data); else reject(data);
  });
}
...
function can_move(monster, based) {
  ...
  var x0 = monster.x, y0 = monster.y, x1 = monster.going_x, y1 = monster.going_y, next, minx = min(x0, x1),
    miny = min(y0, y1), maxx = max(x0, x1), maxy = max(y0, y1);
...
{
  var def = G.maps[map], spawn = def.spawns[door[6]];
  if (point_distance(x, y, spawn[0], spawn[1]) < 40 && can_move({
    map: map,
    x: x,
    y: y,
    going_x: spawn[0],
